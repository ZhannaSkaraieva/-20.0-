...
function defUpperStr1(text) {
  if (text === undefined) { // если параметр отсутствует
    text = 'DEFAULT TEXT';
  }
  return text;
}
console.log(defUpperStr1()); 

function defUpperStr2(text) {
  text = text || 'DEFAULT TEXT';// если значение text ложно или равняется undefined, тогда присвоить text значение 'пусто'
  return text
}
...
function evenFn(n) {
  const result = []; // Створюю порожній рядок, у якому буде розміщено новий створений рядок
  let i = 1; // 0 не повинен потрапляти в результуючий масив,
  while (i <= n) {
    if (i % 2 === 0) {
      result.push(i); // Додаємо нечетное число в массив
    }
    i++; // Увеличиваем i на 1
  }
  return result; // повертаємо массив 
}
...
function ageClassification(n) {
  ...
  console.log('    90 :', ageClassification(90)) // 90 : Довголіття
  console.log(' 90.01 :', ageClassification(90.01)) // 90.01 : Рекорд
  console.log('   120 :', ageClassification(120)) // 120 : Рекорд
  console.log('122.01 :', ageClassification(122.01)) // 122.01 : null
  console.log('   150 :', ageClassification(150)) // 150 : null
...
function oddFn(n) {
  const result1 = []; // Створюю порожній рядок, у якому буде розміщено новий створений рядок
  let i = 1; // 0 не повинен потрапляти в результуючий масив,
  while (i <= n) {
    if (i % 2 !== 0) {
      result1.push(i); // Додаємо нечетное число в массив
    }
    i++; // Увеличиваем i на 1
  }
  return result1; // повертаємо массив 
}
...
function cbRandom(min, max) {  //cbRandom(a, b) - обчислює і повертає довільне ціле число в діапазоні між a і b включно.
  return Math.floor(Math.random() * (max - min + 1)) + min;
}


function cbPow(num, pow) {  // cbPow(a, b) - обчислює і повертає результат піднесення числа a у ступінь b.
  const a = num;
  const b = pow;
  return Math.pow(a, b);
}

function cbAdd(a, b) {  // cbAdd(a, b) - обчислює і повертає суму двох чисел a і b.
  const summ = a + b;
  return summ
 }
